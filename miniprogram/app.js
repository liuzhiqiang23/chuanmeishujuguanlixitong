...
//   3) movie.jszzbinfo.cn 正式地址，服务器上跑着后端；域名备案+证书已就绪，
//                         同学手机上的体验版只有这一条能用（白名单只认 HTTPS 域名）
//   4) 服务器 IP          域名没解析时的过渡通道，IP 进不了合法域名白名单，
//                         只有开发版/预览版（勾了「不校验合法域名」）能用
// 热点重启会换网段，地址 2 可能过期——改了这里重新上传一次即可。
//
// 【别脱敏】这四个地址是程序真要去连的，换成占位符整个小程序就废了。
// 域名和服务器 IP 本来就是公开信息（DNS 一查即得），脱敏它们没有安全收益。
// 真正需要脱敏的只有密钥类：AppSecret / DB 密码 / RSA 私钥（那些在
